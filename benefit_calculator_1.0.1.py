...
while True:
    ...
    if event == "CALCULATE":
        try:
            ...
            def check_commutation():
                ...
                if com > 40:
                    sg.Popup("Commutation must not exceed 40%. Try again.", title="Error!",
                             icon=r'icon.ico')
                    commutation = 100
                    window['pen_report'].update(disabled=True)
                else:
                    commutation = com
                return commutation
            ...
            window['pen_report'].update(disabled=False)
    ...
    if event == "pen_report":
        html_out = template_pen.render(template_var_pension)
        with open(f"pension report.html", "w") as f:
            f.write(html_out)
        f.close()
        sg.webbrowser.open(url="pension report.html", new=2)
    ...
    if event == "cal_gra":
        try:
            ...
            window['gra_report'].update(disabled=False)
    ...
    template_var_gratuity = {"user_name": values['name'], "rg": values['Rgra'], "dg": values['Dgra'],
                             "doj": values['dojPick1'], "dor": values['dorPick1'], "lp": values['total-pay-gra'],
                             "qs": values['xx1'], "sp": values['q_service1']}
    if event == "gra_report":
        html_out = template_gra.render(template_var_gratuity)
        with open(f"gratuity report.html", "w") as f:
            f.write(html_out)
        f.close()
        sg.webbrowser.open(url="gratuity report.html", new=2)
    ...
    if event == "cal_leave":
        try:
            ...
            window['leave_report'].update(disabled=False)
        except:
            sg.Popup("Please enter values in proper format.", title="Error!",
                     icon=r'icon.ico')
            window['leave_report'].update(disabled=True)
    ...
    if event == "leave_report":
        html_out = template_leave.render(template_var_leave)
        with open(f"leave salary report.html", "w") as f:
            f.write(html_out)
        f.close()
        sg.webbrowser.open(url="leave salary report.html", new=2)
